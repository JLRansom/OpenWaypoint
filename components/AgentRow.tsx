...
export function AgentRow({ agent }: { agent: Agent }) {
  ...
  return (
    <tr className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
      <td className="py-3 px-4 font-mono text-xs text-gray-400">{agent.id.slice(0, 8)}</td>
      <td className="py-3 px-4">
        <span className="rounded bg-gray-100 px-2 py-0.5 text-xs font-medium text-gray-700 capitalize">
          {agent.type}
        </span>
      </td>
      <td className="py-3 px-4 max-w-xs truncate text-sm text-gray-700">{agent.prompt}</td>
      <td className="py-3 px-4">
